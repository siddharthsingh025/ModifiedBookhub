...
import com.siddharth.bookhub.adapters.AudioBookRecyclerAdapter
import com.siddharth.bookhub.adapters.FavouriteRecyclerAdapter
import com.siddharth.bookhub.database.BookDatabase
import com.siddharth.bookhub.database.SongDatabase
import com.siddharth.bookhub.database.SongEntity
import kotlinx.android.synthetic.main.fragment_audio_book.view.*
...
class AudioBookFragment : Fragment() {
    ...
    lateinit var recyclerAdapter:AudioBookRecyclerAdapter
...
//    lateinit var contentResolver: ContentResolver
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_audio_book, container, false)
        ...
        if (activity != null) {
            ...
            recyclerAdapter = AudioBookRecyclerAdapter(activity as Context, dbBookList)
...
//                db?.songDao()?.insertSong(SongEntity(title,artist,mp3Data))
                ...
                thread {
                    val db= context?.let { Room.databaseBuilder(it, SongDatabase::class.java,"Song-db").build() }
//
...
class RetrieveFavourite( val context: Context): AsyncTask<Void, Void, List<SongEntity>>(){
    ...
    override fun doInBackground(vararg p0: Void?): List<SongEntity> {
        ...
        val db= Room.databaseBuilder(context, SongDatabase::class.java,"Song-db").build()
